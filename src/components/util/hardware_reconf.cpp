...
namespace allscale { namespace components { namespace util {
    ...
    hardware_reconf::hw_topology hardware_reconf::initial_topo;
    ...
    unsigned long long hardware_reconf::read_system_power(const std::string &sysfs_file)
    {
        ...
        try {
            power = std::stoull(line);
        } catch (const std::invalid_argument& ia)
        {
            std::cerr << "Error reading power sensor: " << ia.what() << ", " << line <<'\n';
        } catch (const std::out_of_range& ofr)
        {
            std::cerr << "Error reading power sensor: " << ofr.what() << ", " << line << '\n';
        } catch ( const std::exception& e )
        {
            std::cerr << "Error reading power sensor: " << e.what() << ", " << line << '\n';
        }
        catch ( ... )
        {
    ...
    {
        hardware_reconf::hw_topology the_topo;
        ...
        the_topo.num_physical_cores = hwloc_get_nbobjs_by_depth(topology, core_depth);
        the_topo.num_logical_cores = std::thread::hardware_concurrency();

//        HPX_ASSERT(topo.num_logical_cores % topo.num_physical_cores == 0);

        the_topo.num_hw_threads = the_topo.num_logical_cores / the_topo.num_physical_cores;
    ...
    //     {
    //         threads[i].join();
    // 	    std::cout << "thread " << i << "joined " << std::flush;
    //     }
    // }
    ...
    void hardware_reconf::make_cpus_online(unsigned int min_cpu_id, unsigned int max_cpu_id)
    {
        ...
        {
            std::string sysfs_cpu_file = boost::str(boost::format(sysfs_file) % cpu_id);
            hardware_reconf::write_to_file( 1, sysfs_cpu_file);
        }
    }
    ...
    {
	hardware_reconf::initial_topo = read_hw_topology();
    }
