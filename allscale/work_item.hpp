...
typename std::enable_if<
		...
		typename hpx::traits::future_traits<F>::result_type>::type unwrap_if(
		F && f) {
	return f.get();
}
...
typename std::enable_if<
		hpx::traits::is_future<F>::value
				&& std::is_same<void,
						typename hpx::traits::future_traits<F>::type>::value
				&& !allscale::traits::is_data_item<F>::value>::type unwrap_if(
		F && f) {
...
struct work_item {
	...
	struct work_item_impl_base: std::enable_shared_from_this<work_item_impl_base> {
...
//             }
		...
		template<typename WorkItemDescription, typename Closure>
		struct work_item_impl<WorkItemDescription, Closure, true, true> : work_item_impl_base {
			std::shared_ptr<work_item_impl> shared_this() {
				return std::static_pointer_cast<work_item_impl>(
						this->shared_from_this());
			}
...
//             }
		...
		template<typename WorkItemDescription, typename Closure>
		struct work_item_impl<WorkItemDescription, Closure, false, true> : work_item_impl_base {
			std::shared_ptr<work_item_impl> shared_this() {
				return std::static_pointer_cast<work_item_impl>(
						this->shared_from_this());
			}
			...
			using data_item_type = typename allscale::data_item<data_item_descr>;
	/*
            const std::vector<allscale::data_item_base>& requires() {
			//				std::cout << "call to requiries" << std::endl;
			//				std::vector<allscale::data_item_base>* bla = new std::vector<allscale::data_item_base>();
							return std::move(std::vector<allscale::data_item_base>());
						}
    */
			work_item_impl() {
			}
...
//             }
		...
		template<typename WorkItemDescription, typename Closure>
		struct work_item_impl<WorkItemDescription, Closure, false, false> : work_item_impl_base {
			std::shared_ptr<work_item_impl> shared_this() {
				return std::static_pointer_cast<work_item_impl>(
						this->shared_from_this());
			}
...
//            bool DataItemUsage = WorkItemDescription::data_item_variant::activated
//        >
//        struct work_item_impl;
//
//        template <typename WorkItemDescription, typename Closure>
//        struct work_item_impl<WorkItemDescription, Closure, true, true, true>
//          : work_item_impl_base
//        {
//            std::shared_ptr<work_item_impl> shared_this()
//            {
...
//============================
		...
		impl_(
				new work_item_impl<WorkItemDescription,
				hpx::util::tuple<
				typename hpx::util::decay<
				decltype(detail::futurize_if(std::forward<Ts>(vs)))>::type...> >(
						std::move(tre),
						detail::futurize_if(std::forward<Ts>(vs))...)), is_first_(
				is_first) {
			impl_->set_this_id();
		}
