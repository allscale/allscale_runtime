...
struct grid_init_process {
    ...
    get_requirements(Closure const& c)
    {
        ...
        region_type r(
            hpx::util::get<1>(c) - coordinate_type(1),
            hpx::util::get<2>(c) + coordinate_type(1)
        );
...
struct main_process
{
    static allscale::treeture<int> execute(hpx::util::tuple<> const& c)
    {
        ...
        region_type whole_region({1,1}, {199, 199});

        coordinate_type begin(1, 1);
        coordinate_type end(199, 199);
